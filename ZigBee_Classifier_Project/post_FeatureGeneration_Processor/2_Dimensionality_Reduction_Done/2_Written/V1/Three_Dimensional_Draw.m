function Three_Dimensional_Draw ( fig_1,                                                ...
                                fig_2,                                                ...
    ...
    % Level 1: Drawing the 'Pure Data-Points' 
               % of both 'Scatter Plot' and 'Projection Plot'
                ...
                % Stage 1: Scatter Plot
                    figure ( fig_1 );
                    different_Labeling_Sweep_Cases = combntns( 1 : size ( projected_DataPoints, 1 ), 3 );
                    ...
                    for class_Index = 1 : size ( different_Classes, 2 )
                        ...
                        Z = current_Class_of_pure_DataPoints ( selected_Dimensions_for_Draw ( 1, 3 ), : ); 

                        h (1, class_Index ) = plot3( X, Y, Z, 'o', 'Color', colors_for_DataPoints ( class_Index, : ),  'MarkerSize', 10, 'DisplayName',sprintf ('Class: %d', different_Classes (1, class_Index) ));            
                        hold on
                    ...
                    % Plotting the 'Component's Direction Lines'                                
                        for column_Index = 1 : size ( selected_Dimensions_for_Draw, 2 )                                         
                            h (1, class_Index + column_Index ) = plot3([0 V( 1, column_Index )],[0 V( 2, column_Index )], [0 V( 3, column_Index )], 'Color', colors_for_Lines ( column_Index, : ), 'LineWidth', 3, 'DisplayName', sprintf ('Projection-Direction: %d', column_Index ) );
                    ...
                    if ( isempty ( axis_Labels ) == 0 )
                        ...
                        y_Label = sprintf('%s%s', axis_Labels{2}, additional_Y_Label );
                        z_Label = sprintf('%s%s', axis_Labels{3}, additional_Z_Label );
                        ylabel( y_Label, 'Rotation', 0 );
                        zlabel( z_Label, 'Rotation', 0 );
                ...
                % Stage 1: Scatter Plot
                    ...
                    for dimension_Index = 1 : maximum_Dimensions
                        ...
                        for class_Index = 1 : size ( different_Classes, 2 )
                            ...
                            Z = current_Class_of_projected_DataPoints_for_Selected_Dimensions ( 3, : );                                       

                            dimensions_Text = sprintf ( '%d ', row_Indices);
                            dimensions_Text = sprintf ( 'Auto. Projection Dimensions : [%s]', dimensions_Text(1, 1 : end - 1));
                            h (1, class_Index ) = plot3( X, Y, Z, 'o', 'Color', colors_for_DataPoints ( class_Index, : ),  'MarkerSize', 10, 'DisplayName',[ dimensions_Text sprintf(' -- Class: %d', different_Classes (1, class_Index) ) ] );             %#ok<*SAGROW>
                            hold on
                        end
                        ...
                        x_Label = sprintf ( 'Projected \n DataPoints (%d, :)', row_Indices ( 1, 1 ) );
                        xlabel( x_Label, 'Rotation', 0 )
                        
                        y_Label = sprintf ( 'Projected \n DataPoints (%d, :)', row_Indices ( 1, 2 ) );
                        ylabel( y_Label, 'Rotation', 0 )
                        
                        z_Label = sprintf ( 'Projected \n DataPoints (%d, :)', row_Indices ( 1, 3 ) );
                        zlabel( z_Label, 'Rotation', 0 )
