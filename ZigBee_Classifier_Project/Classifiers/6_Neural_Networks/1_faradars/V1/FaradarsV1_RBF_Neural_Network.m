function output = FaradarsV1_RBF_Neural_Network ( varargin )
    ...
    %% Section 1: Extraction of Essential Parameters
        % Level 1: Extraction of Input
            ...
            if        ( isempty( input_DataPoints_Matrix ) == 1 )        
                error ( 'You should Enter the "input_DataPoints_Matrix" for Classification in "FaradarsV1_RBF_Neural_Network".' );

            elseif    ( isempty( classLabels_from_DataBank ) == 1 )        
                error ( 'You should Enter the "classLabels_from_DataBank" for Classification in "FaradarsV1_RBF_Neural_Network".' );
            ...
            if ( isempty( special_Structure_of_Parameters_for_Classification ) == 0 && isempty ( isfield ( special_Structure_of_Parameters_for_Classification, 'number_of_Hidden_Layer_Neurons' ) ) == 0 )
                numberMAx_of_Hidden_Layer_Neurons   = special_Structure_of_Parameters_for_Classification.numberMAx_of_Hidden_Layer_Neurons;
                
            else
                numberMAx_of_Hidden_Layer_Neurons = 3;
                
            end                       
            
            if ( isempty( special_Structure_of_Parameters_for_Classification ) == 0 && isempty ( isfield ( special_Structure_of_Parameters_for_Classification, 'Saving_Address' ) ) == 0 )
            ...
            if ( isempty( special_Structure_of_Parameters_for_Classification ) == 0 && isempty ( isfield ( special_Structure_of_Parameters_for_Classification, 'selected_Saving_Extension' ) ) == 0 )
    ...
    %% Section 2: Classification 
       % Stage 1: Normalization
        ...
        % Create a Fitting Network
            hiddenLayerSize = numberMAx_of_Hidden_Layer_Neurons;
            Goal=0;
            Spread=1;
            % MaxNeuron=10;
            DisplayAt=1;
            net = newrb(inputs,targets,Goal,Spread,hiddenLayerSize,DisplayAt);
...
%             testPerformance  = perform(net, testTargets, outputs);
    ...
    %% Section 3: Demonstration
        ...
        % Making the Proper Name
            ...
            for index = 1 : size ( numberMAx_of_Hidden_Layer_Neurons, 2 )
                number_of_Hidden_Layer_Neurons_Text = [ number_of_Hidden_Layer_Neurons_Text '_' num2str( numberMAx_of_Hidden_Layer_Neurons(1, index) ) ];
            end
