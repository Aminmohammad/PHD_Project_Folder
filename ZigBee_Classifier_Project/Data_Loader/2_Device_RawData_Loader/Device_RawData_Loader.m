function Vertical_Structure_of_all_Bursts = Device_RawData_Loader ( device_DatSet_Address, zero_Conversion_Threshold, number_of_subRegions )
    ...
    for records_Index = 1 : size ( list_of_Records, 1 )
        ...
        % Extracting the 'Bursts' and 'subRegions' of a Single Record
            ...
            for burst_Index = 1 : size ( bursts_Indices_Matrix, 1 )
                ...
                % Single Burst
                    current_Burst = content_of_Current_Record( 1, bursts_Indices_Matrix(burst_Index, 1 ) : bursts_Indices_Matrix(burst_Index, 2 ) );
                    current_Burst = current_Burst(:);
                    
                    if size ( current_Burst, 1 ) < number_of_subRegions
                        current_Burst = repmat( current_Burst, number_of_subRegions, 1 );
                        
                    end

                % subRegionsof 'current_Burst'
                    length_of_a_Single_subRegion = floor(size ( current_Burst, 1 )/ number_of_subRegions );
                    ...
                    for subRegion_Index = 1 : number_of_subRegions + 1
                        if ( subRegion_Index <= number_of_subRegions )
                             ...
                             temp = current_Burst ( indices, 1 );  
                             vertical_Structure_of_a_Single_Burst ( subRegion_Index, 1 ).a_Single_subRegion = temp;

                        else

                             vertical_Structure_of_a_Single_Burst ( subRegion_Index, 1 ).a_Single_subRegion =  current_Burst;
                        end
